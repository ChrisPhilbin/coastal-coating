    ...
    <Form @submit="handleKioskSubmit" v-slot="{ errors }" :validation-schema="schema" class="ml-auto mr-auto w-3/4">
      ...
      <div class="w-full block flex text-center p-4">
        <div class="w-full block text-center">
          ...
          >
            {{ inSubmission ? `Please wait...` : `Submit` }}
          </button>
        </div>
      </div>
    </Form>
  </div>
  <KioskThankYou v-if="formSuccessWithoutErrors" />
</template>
...
import KioskThankYou from "../kiosk-thank-you/Kiosk-Thank-You.vue";

export default {
  name: "KioskForm",
  props: ["discountEarned"],
  components: { Form, Field, KioskThankYou },
  mixins: [formMixin],
  setup() {
    const phoneRegEx =
  ...
  data() {
    return {
      inSubmission: false,
      formSuccessWithoutErrors: false,
      appointmentDetails: {
        firstName: "",
        lastName: "",
        phone: "",
  ...
  methods: {
    ...
    submitSuccess() {
      this.inSubmission = false;
      this.formSuccessWithoutErrors = true;
      setTimeout(() => {
        this.$router.go();
      }, 10000);
    },
